...
# --- 🌐 0. ระบบจัดการภาษา (Dictionary) ---
LANG_DICT = {
    "Thai": {
        "settings": "⚙️ ตั้งค่า",
        "lang_label": "เลือกภาษา",
        "free_mode": "โหมดใช้งานฟรี",
        "theme_label": "โหมดมืด (Dark Mode)",
        "logout": "ออกจากระบบ",
    ...
    "English": {
        ...
        "run": "✨ Run Analysis",
        "temp": "🌡️ Avg Temp",
        "warn": "⚠️ **Weather: Extreme Cold** | Prepare winter gear",
        "analysis_title": "🔍 Outfit Analysis",
        "ai_img": "🎭 AI Simulation",
        "essential_title": "📋 Essentials List",
        "shop_title": "🛍️ Shopping Links",
        "essentials": [
            "Heavy Down Jacket", "Fleece Lined Pants", "Beanie & Scarf", "Winter Boots", "Hot Packs & Gloves"
        ]
    }
}
...
def main_dashboard():
    ...
    with col2:
        if run_btn:
            ...
            st.info("👈 " + ("Please run analysis" if lang=="English" else "กรุณากดเริ่มวิเคราะห์"))

# --- 🔑 3. หน้า Login ---
def login_page():
    st.markdown("""<style>
        div.stButton > button { height: 55px; width: 100%; background-color: transparent !important; color: transparent !important; border: 1px solid #dadce0 !important; border-radius: 8px !important; position: relative; z-index: 2; }
        .login-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 55px; display: flex; align-items: center; justify-content: center; border-radius: 8px; z-index: 1; pointer-events: none; border: 1px solid #dadce0; background: white; }
        .login-text { font-family: sans-serif; font-weight: 500; font-size: 16px; color: #3c4043; }
    </style>""", unsafe_allow_html=True)

    st.title("🌍 Tripnify Login")
    st.markdown("---")
    
    g_logo = "https://www.gstatic.com/images/branding/product/1x/gsa_512dp.png"
    fb_logo = "https://upload.wikimedia.org/wikipedia/commons/b/b8/2021_Facebook_icon.svg"

    # Google
    st.markdown(f'<div style="position: relative;"><div class="login-overlay"><img src="{g_logo}" width="20px" style="margin-right:12px;"><span class="login-text">เข้าสู่ระบบด้วยบัญชี Google</span></div></div>', unsafe_allow_html=True)
    if st.button("g", key="g_btn"): st.session_state['logged_in'] = True; st.rerun()

    st.write("")
    # Facebook
    st.markdown(f'<div style="position: relative;"><div class="login-overlay"><img src="{fb_logo}" width="20px" style="margin-right:12px;"><span class="login-text">เข้าสู่ระบบด้วยบัญชี Facebook</span></div></div>', unsafe_allow_html=True)
    if st.button("f", key="f_btn"): st.session_state['logged_in'] = True; st.rerun()
