    ...
    with col2:
      with col2:
        ...
        if run_btn:
            ...
            # --- 3. ส่วนที่ปรับปรุง: สิ่งที่ควรเตรียมเพิ่มเติม ---
            st.markdown("### 📋 สิ่งที่ควรเตรียมเพิ่มเติม")
            
            # รายการ 5 ข้อหลักที่ดึงมาจากหัวข้อวิเคราะห์ ให้ตรงกับสินค้าที่จะช้อปปิ้ง
            essentials = [
                "เสื้อโค้ทกันหนาวหนาพิเศษ (Padding Jacket)",
                "กางเกงบุขนกันหนาว (Fleece Lined Pants)",
                "หมวกไหมพรมและผ้าพันคอ (Winter Accessories)",
                "รองเท้าบูทกันหนาวหนาพิเศษ (Winter Boots)",
            ...
            # ปรับปรุงข้อความและข้อมูลด้านในตามสั่ง
            with st.expander("คลิกเพื่อดูรายละเอียดสิ่งที่เตรียมเพิ่มเติมแต่ละอันว่ามีประโยชน์อย่างไรประกอบการตัดสินใจ"):
                st.info(r_out)

            # --- 4. ส่วนแหล่งช้อปปิ้งแนะนำ (ปรับให้สินค้าตรงกับด้านบนมากที่สุด) ---
            st.markdown("### 🛍️ แหล่งช้อปปิ้งแนะนำ")
            for it in essentials:
                # แยกชื่อภาษาไทยออกมาเพื่อใช้เป็น Keyword ในการค้นหาให้แม่นยำที่สุด
                search_term = it.split('(')[0].strip()
                st.markdown(f"""
                <div class="shop-card">
                    <strong>🛒 สั่งซื้อเพิ่ม: {search_term}</strong><br>
                    <small>ค้นหาสินค้าที่สอดคล้องกับผลวิเคราะห์ของคุณ</small><br>
                    <a href='https://shopee.co.th/search?keyword={quote_plus(search_term)}' target='_blank'>🛍️ ค้นหาใน Shopee</a> | 
                    <a href='https://www.lazada.co.th/catalog/?q={quote_plus(search_term)}' target='_blank'>🛍️ ค้นหาใน Lazada</a>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("👈 กรุณากรอกข้อมูลการเดินทางและกดปุ่มเริ่มวิเคราะห์เพื่อดูผลลัพธ์")
def login_page():
