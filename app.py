...
def main_dashboard():
    ...
    with col2:
        if run_btn:
            ...
            # --- 3. สิ่งที่ควรเตรียมเพิ่มเติม (ปรับปรุงตามสั่ง) ---
            st.markdown("### 📋 สิ่งที่ควรเตรียมเพิ่มเติม")
            
            # รายการสินค้า 5 อย่างหลักที่สอดคล้องกับอากาศ 1.8°C และการช้อปปิ้ง
            essentials = [
            ...
            # แสดงรายการ 5 ข้อหลัก
            for i, item in enumerate(essentials, 1):
                st.write(f"{i}. **{item}**")
            
            # Expander พร้อมข้อความใหม่ตามที่คุณกำหนด
            with st.expander("คลิกเพื่อดูรายละเอียดสิ่งที่เตรียมเพิ่มเติมแต่ละอันว่ามีประโยชน์อย่างไรประกอบการตัดสินใจ"):
                st.info(r_out)

            st.divider()

            # --- 4. แหล่งช้อปปิ้งแนะนำ (ดึงชื่อสินค้าจากรายการด้านบนมาใช้โดยตรง) ---
            st.markdown("### 🛍️ แหล่งช้อปปิ้งแนะนำ")
            for it in essentials:
                # ใช้ชื่อไทยส่วนแรกเป็น Keyword ในการค้นหาเพื่อให้ผลลัพธ์แม่นยำที่สุด
                search_term = it.split('(')[0].strip()
                st.markdown(f"""
                <div class="shop-card">
                    <strong>🛒 สั่งซื้อเพิ่ม: {search_term}</strong><br>
                    <small>ค้นหาสินค้าที่สอดคล้องกับคำแนะนำสำหรับอากาศหนาวจัด</small><br>
                    <a href='https://shopee.co.th/search?keyword={quote_plus(search_term)}' target='_blank'>🛍️ Shopee</a> | 
                    <a href='https://www.lazada.co.th/catalog/?q={quote_plus(search_term)}' target='_blank'>🛍️ Lazada</a>
                </div>
                """, unsafe_allow_html=True)
        else:
            # ข้อความแสดงเมื่อยังไม่ได้กดวิเคราะห์
            st.info("👈 กรุณากรอกข้อมูลการเดินทางและอัปโหลดรูปชุดเพื่อเริ่มการวิเคราะห์")
def login_page():
